-- Fix: update_league_standings() used TRUNCATE/INSERT on league_standings (a VIEW).
-- Targets league_standings_new and rebuilds gw1..gw38 + totals from gw_scores.
-- Applied via migration on FPL 2; re-run if you recreate the database.
...
-- Statement-level trigger: must return NULL (not NEW).
...
AS $function$
BEGIN
  PERFORM public.update_league_standings();
  RETURN NULL;
END;
$function$;
